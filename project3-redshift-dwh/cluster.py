...
def main():
    ...
    DWH_DB = config.get("REDSHIFT", "DWH_DB")
    DWH_DB_USER = config.get("REDSHIFT", "DWH_DB_USER")
    DWH_DB_PASSWORD = config.get("REDSHIFT", "DWH_DB_PASSWORD")
    DWH_PORT = config.get("REDSHIFT", "DWH_PORT")

    DWH_IAM_ROLE_NAME = config.get("REDSHIFT", "DWH_IAM_ROLE_NAME")
    ...
    create_cluster(redshift, roleArn, DWH_CLUSTER_TYPE, DWH_NODE_TYPE,
    ...
    myClusterProps, DWH_ENDPOINT = get_cluster_props(
        redshift, DWH_CLUSTER_IDENTIFIER)

    open_ports(ec2, myClusterProps, DWH_PORT)

    conn = psycopg2.connect("host={} dbname={} user={} password={} port={}".format(
        *config['CLUSTER'].values(), host=DWH_ENDPOINT))
    cur = conn.cursor()
